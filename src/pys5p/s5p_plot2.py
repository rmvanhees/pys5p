...
class S5Pplot:
    ...
    def __add_hkdata(i_ax, axarr, hk_data, hk_keys):
        ...
        for key in hk_keys:
            if key not in hk_data.values.dtype.names:
                continue

            indx = hk_data.values.dtype.names.index(key)
    ...
    def draw_trend1d(self, msm1, hk_data=None, msm2=None, *, hk_keys=None,
        ...
        hk_data   :  xarray.DataArray, optional
        ...
        if hk_data is not None:
            ...
            if hk_keys is None:
                if 'detector_temp' in hk_data.values.dtype.names:
                    hk_keys = ('detector_temp', 'grating_temp',
                               'imager_temp', 'obm_temp')
                elif 'temp_det4' in hk_data.values.dtype.names:
                    hk_keys = ('temp_det4', 'temp_obm_swir_grating')
                else:
                    hk_keys = tuple(hk_data.values.dtype.names)[0:4]
            npanels += len(hk_keys)
