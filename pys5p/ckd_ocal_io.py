...
class CKDio():
    ...
    def __init__(self, ckd_dir='/nfs/Tropomi/ocal/ckd'):
        ...
        if not self.__ckd_dir.is_dir():
            raise FileNotFoundError(
                'CKD base-directory does not exist: {}'.format(ckd_dir))
        if not (self.__ckd_dir / 'ckd_release').is_dir():
            raise FileNotFoundError(
                'UVN CKD subdirectory does not exist: {}'.format(ckd_dir))
        if not (self.__ckd_dir / 'ckd_release_swir').is_dir():
            raise FileNotFoundError(
                'SWIR CKD subdirectory does not exist: {}'.format(ckd_dir))
    ...
    def get_swir_absirr(self, qvd=1):
        ...
        if not ckd_dir.is_dir():
            raise FileNotFoundError('{} does not exist'.format(ckd_dir))

        file_b7 = ckd_dir / 'irrad_conv_factors.band7.ckd.nc'
        if not file_b7.is_file():
            raise FileNotFoundError('CKD {} not found'.format(file_b7))
        file_b8 = ckd_dir / 'irrad_conv_factors.band8.ckd.nc'
        if not file_b8.is_file():
            raise FileNotFoundError('CKD {} not found'.format(file_b8))
    ...
    def get_swir_darkflux(self):
        ...
        if not file_ch4.is_file():
            raise FileNotFoundError('CKD {} not found'.format(file_ch4))
    ...
    def get_swir_dpqf(self, threshold=None):
        ...
        file_ch4 = ckd_dir / 'ckd.dpqf.detector4.nc'
        if not file_ch4.is_file():
            raise FileNotFoundError('CKD {} not found'.format(file_ch4))
    ...
    def get_swir_v2c(self):
        ...
        file_ch4 = ckd_dir / 'ckd.v2c_factor.detector4.nc'
        if not file_ch4.is_file():
            raise FileNotFoundError('CKD {} not found'.format(file_ch4))
